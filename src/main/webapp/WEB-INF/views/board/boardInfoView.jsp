            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid">
                        <h1 class="mt-4">게시판관리 페이지</h1>
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">원하시는 게시판을 생성, 삭제, 표시 해보세요!</li>
                        </ol>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                	<form action="${cp}/board/registBoardInfo" method="post">
	                                	게시판 이름 :&nbsp;
	                                	<input type="text" id = "boardName" name="boardName" value="">
	                                	<select name="check">
	                                		<option value="t">사용</option>
	                                		<option value="f">미사용</option>
	                                	</select>
	                                	<input type="submit" id = "registBoardInfo" value="생성"><br>
                                	</form>
                               		<c:forEach items="${boardInfoList}" var="boardinfo">
                               			<form action="${cp}/board/modifyBoardInfo" method="post">
		                           			게시판 이름	 :&nbsp;
		                           			<input type="hidden" name="boardNo" value="${boardinfo.bor_no}">
		                           			<input type="text" value="${boardinfo.bor_name}" readonly> 
		                           			<select name="check">
		                           				<c:choose>
		                           					<c:when test="${boardinfo.bor_act==1}">
					                               		<option value="t" selected="selected">사용</option>
					                               		<option value="f" >미사용</option>
				                               		</c:when>
				                               		<c:otherwise>
				                               			<option value="t" >사용</option>
					                               		<option value="f" selected="selected">미사용</option>
				                               		</c:otherwise>
			                               		</c:choose>
		                               		</select>
